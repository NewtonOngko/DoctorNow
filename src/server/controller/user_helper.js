...
exports.create = function addUser(req, res, email) {
  ...
  const data = User.getEmail((err, email) => {
    for (var i = 0; i < email.length; i++) {
      console.log('res', Object.values(JSON.parse(JSON.stringify(email[i]))))
      if (newUser.email == Object.values(JSON.parse(JSON.stringify(email[i])))) {
        res.status(202).send({ error: true, message: 'email has been used' })
      }
    }
  })
