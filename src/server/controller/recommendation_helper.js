...
exports.findAll = function getRecommendation(req, res) {
  Recommendation.getMatriksKeputusan().then(async (resData) => {
    let newData = await prosesData(resData);
    console.log(newData);
    Recommendation.postEDASAlgorithm(newData).then((responsePost) => {
...
const prosesData = async (data) => {
  ...
  const arrAS = AS.map(({ info, score }, index) => {
    let temp = [];
    temp.push(index + 1);
